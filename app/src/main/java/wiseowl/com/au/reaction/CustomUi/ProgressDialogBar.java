package wiseowl.com.au.reaction.CustomUi;
...
/**
 * Created by anish.patel on 6/11/2016.
 */



public class ProgressDialogBar extends View {
    public static final String TAG = ProgressDialogBar.class.getSimpleName();
    ...
    private ArrayList<DotDrawable> mDrawables = new ArrayList<>();
    ...
    public ProgressDialogBar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ProgressDialogBar(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs);
    }

    private void init(AttributeSet attrs) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.DotsProgressBar);
        mNumberDots = a.getInt(R.styleable.DotsProgressBar_dd_numDots, 3);
        mDotRadius = a.getDimension(R.styleable.DotsProgressBar_android_radius, 8);
        mDotColor = a.getColor(R.styleable.DotsProgressBar_android_color, 0xff9c27b0);
        mDotScaleMultiplier = a.getFloat(R.styleable.DotsProgressBar_dd_scaleMultiplier, DEFAULT_GROWTH_MULTIPLIER);
        mAnimationDuration = a.getInt(R.styleable.DotsProgressBar_dd_animationDuration, 300);
        mHorizontalSpacing = a.getDimension(R.styleable.DotsProgressBar_dd_horizontalSpacing, 12);
        a.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        if (shouldAnimate()) {
            for (DotDrawable dot : mDrawables) {
                dot.draw(canvas);
            }
        }
    }
    ...
    private void initDots() {
        ...
        for (int i = 1; i <= mNumberDots; i++) {
            final DotDrawable dot = new DotDrawable(mDotColor, mDotRadius, mDotMaxRadius);
    ...
    private void updateDots() {
        ...
        for (int i = 0; i < mDrawables.size(); i++) {
            final DotDrawable dot = mDrawables.get(i);
